...
usually don't bother with them but on those occasions when my life gets a
bit hectic (it's not too often) I will sometimes make use of one.  When that
...
The end result is an unsorted list of things you can do in the moment, all
in the service of the various goals you have decided to put on your radar.
...
this regard is Org, a note keeping and text outlining tool that has evolved
over the years to include task management and scheduling functionality.
...
caveats that I'll get into shortly [XXX if this is about the heading level,
maybe not such a big deal].  Viewing my next actions, waiting, projects and
someday lists is easily accomplished with org-agenda, again with some
qualifications that I will get into shortly [XX what caveats?].  Contexts,
and viewing the next actions attached to them, are easily implemented with
Org tags and the narrowing function functionality of org-agenda.
...
Your Next Actions, for example, could live in an actions.org file that looks
like this:
...
same vein, a project will often have at least one next action linked it - but
...
The GTD community sometimes takes this even further - they often don't
recommend *any* explicit connection between next actions and projects at
all.  Projects, in this formulation, simply act as a reminder that a next
action need to be created, but once that next action is in your system you
don't (so the theory goes) need to know which project spawned it.
...
to know which project a next action belongs under.  And the thing is,
...
For example, Org has a feature called drawers that let you add arbitrary
properties to TODO items.  Add a Project ID to each project item, and a
Project ID to each Next Action that belongs to a project and you have a
theoretical way of connecting the two lists.
...
view and write the code to produce - not the easiest of tasks.
...
it.  Org certainly doesn't do this out of the box, and honestly I'm having
trouble imagining what that feature would even look like.
...
At this point, it makes sense to combine the projects and next actions into
one file, so all your next actions are located in just one place:
...
You could integrate your someday list into this single list, but I actually
...
with org-refile.  Archiving is also trivial, as long as one sticks to the
top level items, i.e. as long as one doesn't try to archive unfinished
projects.
